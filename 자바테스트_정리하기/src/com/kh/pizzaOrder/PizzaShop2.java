package com.kh.pizzaOrder;

import java.util.ArrayList;
import java.util.HashSet;
import java.util.List;
import java.util.Set;

public class PizzaShop2 {

	public static void main(String[] args) {
		//주문 목록을 저장할 ArrayList
		List<PizzaOrder> 주문리스트 = new ArrayList<>();
		
		//기존
		PizzaOrder 주문1 = new PizzaOrder("고영희","페페로니");
		주문리스트.add(주문1);
		
		//위에서 작성한 주문 리스트와 아래에 작성한 주문 리스트는 작성방법이 다를 뿐
		//동일하게 주문 리스트에서 주문을 추가하는 방법
		
		//PizzaOrder 필수 생성자 이용해서 바로 입력
		주문리스트.add(new PizzaOrder("박철진","마르게리따"));
		주문리스트.add(new PizzaOrder("안영식","슈프림"));
		주문리스트.add(new PizzaOrder("고영희","파인애플"));
		주문리스트.add(new PizzaOrder("박철진","치즈"));

		//주문한 내용 전체보기 toString
		System.out.println(주문리스트);
		//toString은 추가로 어떤 출력을 하지 않아도 PizzaOrder에 작성한 자체가
		//정보를 정확하게 출력한다는 의미로 표기됨
		//1개씩 보고 싶다면
		for(PizzaOrder 리스트 : 주문리스트) {
			System.out.println(리스트);
		}
		
		//HashSet 사용해서 중복 주문 방지하기 위한 set
		ArrayList<PizzaOrder> 중복주문제거 = new ArrayList<>();
		Set<String> 중복고객 = new HashSet<>();
		for(PizzaOrder 주문 : 주문리스트) {
			if(!중복고객.contains(주문.get고객명())) {
				중복주문제거.add(주문);
				중복고객.add(주문.get고객명());
			}
			
		}
		
		System.out.println(중복주문제거);
		//중복된 주문 정보 제거한 목록 출력
	}

}
